...
public class FilaCaixaManager : MonoBehaviour
{
    ...
    private void Update()
    {
        // Limpa entradas inválidas (objetos desativados/devolvidos ao pool) e reorganiza fila
        int antes = clientesNaFila.Count;
        clientesNaFila.RemoveAll(c => c.cliente == null || !c.cliente.activeInHierarchy);
        if (clientesNaFila.Count < antes)
        {
            AvancarFila();
        }
    }
