...
public class UpgraderCaixa : MonoBehaviour
{
    [Header("Referência ao Pacote")]
    public Caixa caixaAlvo;

    public CaixaVendaTrigger caixaVendaTrigger;
    ...
    {
        custoAtual = Mathf.FloorToInt(custoInicial * Mathf.Pow(caixaAlvo.GetNivelDoCaixa(), expoenteCusto));
        pagoAtual = 0;
    }
    ...
    void AtualizarUI()
    {
        ...
        if (nivelTexto != null)
            nivelTexto.text = "Nível: " + caixaAlvo.GetNivelDoCaixa();
    }
    ...
    void AplicarUpgrade()
    {
        caixaAlvo.nivelDocaixa++;

        caixaAlvo.capacidadeBebida = Mathf.FloorToInt(caixaAlvo.capacidadeBebida * multiplicadorCapacidade);
        caixaAlvo.capacidadeItem = Mathf.FloorToInt(caixaAlvo.capacidadeItem * multiplicadorCapacidade);
        caixaVendaTrigger.intervaloVenda -= 0.1f;

        Debug.Log($"Upgrade aplicado! Novo nível: {caixaAlvo.GetNivelDoCaixa()} | Capacidade: {caixaAlvo.GetCapacidadeBebida()} | Velocidade: {caixaAlvo.GetCapacidadeItem()}");
    }
