...
public class UpgraderFreezer : MonoBehaviour
{
    [Header("Referência ao Freezer")]
    public Freezer freezerAlvo;
    ...
    void Start()
    {
        if (freezerAlvo == null)
        {
            Debug.LogError("Freezer alvo não foi atribuído no Upgrader!");
            enabled = false;
            return;
        }

        freezerAlvo.GetNivelDoFreezer();
    ...
    {
        custoAtual = Mathf.FloorToInt(custoInicial * Mathf.Pow(freezerAlvo.GetNivelDoFreezer(), expoenteCusto));
        pagoAtual = 0;
    }
    ...
    void AtualizarUI()
    {
        ...
        if (nivelTexto != null)
            nivelTexto.text = "Nível: " + freezerAlvo.GetNivelDoFreezer();
    }
    ...
    void AplicarUpgrade()
    {
        freezerAlvo.nivelDoFreezer++;

        freezerAlvo.capacidadeFreezer = Mathf.FloorToInt(freezerAlvo.capacidadeFreezer * multiplicadorCapacidade);
        freezerAlvo.velocidadeCongelamento = freezerAlvo.velocidadeCongelamento * multiplicadorVelocidade;

        Debug.Log($"Upgrade aplicado! Novo nível: {freezerAlvo.GetNivelDoFreezer()} | Capacidade: {freezerAlvo.GetCapacidadeMaxima()} | Velocidade: {freezerAlvo.GetVelocidadeCongelamento()}");
    }
