...
public class UpgraderPacoer : MonoBehaviour
{
    [Header("Referência ao Pacote")]
    public BalcaoEmpacotar balcao;
    ...
    void Start()
    {
        if (balcao == null)
        {
            Debug.LogError("Balcão alvo não foi atribuído no Upgrader!");
            enabled = false;
            return;
        }

        balcao.GetNivelDoBalcao();
    ...
    {
        custoAtual = Mathf.FloorToInt(custoInicial * Mathf.Pow(balcao.GetNivelDoBalcao(), expoenteCusto));
        pagoAtual = 0;
    }
    ...
    void AtualizarUI()
    {
        ...
        if (nivelTexto != null)
            nivelTexto.text = "Nível: " + balcao.GetNivelDoBalcao();
    }
    ...
    void AplicarUpgrade()
    {
        balcao.nivelDoBalcao++;

        balcao.capacidadeProduto = Mathf.FloorToInt(balcao.capacidadeProduto * multiplicadorCapacidade);
        balcao.capacidadeEmpacotado = Mathf.FloorToInt(balcao.capacidadeEmpacotado * multiplicadorCapacidade);

        Debug.Log($"Upgrade aplicado! Novo nível: {balcao.GetNivelDoBalcao()} | Capacidade: {balcao.GetCapacidadeProduto()} | Velocidade: {balcao.GetCapacidadeEmpacotado()}");
    }
